...
/**
 * File: BleConstants.java
 * Author: yuzhuzhang
 * Create: 2020-01-01 20:48
 * Description: TODO 披克Ble设备中通信的命令/参数 常量类
 * 不要随意修改
 * -----------------------------------------------------------------
 * 2020-01-01 : Create BleConstants.java (yuzhuzhang);
 * -----------------------------------------------------------------
 */
public interface BleConstants {

    //披克BLE蓝牙ServiceUUID
    String PEAKE_SERVICE_UUID = "0000ffe0-0000-1000-8000-00805f9b34fb";
    //披克BLE蓝牙CharacterUUID
    String PEAKE_CHARACTER_UUID = "0000ffe1-0000-1000-8000-00805f9b34fb";
    //披克蓝牙设备类型: M :门禁 K:考勤 X:消费 ;  目前是设备的名字中的开头的第三个英文字母为类型
    String[] BLE_TYPE = {"M", "K", "X"};
    //披克门禁 BLE蓝牙设备名开头固定-字母
    String PEAKE_BLEDEVICE_PRE_M_NAME = "PKM";
    //披克所用的BLE蓝牙设备
    String PEAKE_BLEDEVICE_PRE_NAME = "PK";

    //披克BLE蓝牙设备 信号过滤阈值
    double PEAKE_MIN_RSSI_VALUE = 0.45;

    //
    byte BLE_READE_ERROR_MSG_KEY = (byte) 0xFF;
    //链路检查-->正常时候读卡器也不会响
    byte BLE_CHECK_LINK_MSG_KEY = (byte) 0x01;
    //客户端主动发出的加密秘钥【类似握手秘钥】
    byte BLE_ENCRYPT_KEY_MSG_KEY = (byte) 0x8;
    //客户端主动发送加密后的卡号
    byte BLE_CARD_ID_MSG_KEY = (byte) 0xA;
    //客户端修改蓝牙模块名称和发射功率
    byte BLE_SET_BLEINFO_MSG_KEY = (byte) 0x6;


    //读卡器回送加密密钥
    byte BLE_RESPONSE_SHAKE_FLAGE = (byte) 0x9;
    //开门状态 发送卡号后返回数据
    byte BLE_RESPONSE_CARD_FLAGE = (byte) 0x4;
    //蓝牙设备消息 回执结果--> 操作正确   [开门状态 --> 成功]
    byte BLE_RESPONSE_SUCCESS = (byte) 0x00;
    //蓝牙设备消息 回执结果--> 操作失败 未知错误   [开门状态 --> 失败]
    byte BLE_RESPONSE_FAIL = (byte) 0x01;
    // 蓝牙设备消息回执命令
    byte BLE_RESPONSE_FLAGE = (byte) 0x02;
    //  CRC错误
    byte BLE_RESPONS_FAIL_CRC = (byte) 0xF1;


    /*披克BLE蓝牙 8个字节长度固定对信息 加密公钥 key*/
    byte[] PEAKE_BLE_ENCRYPT_KEY = {
            (byte) 0xB6, (byte) 0xF2, (byte) 0xd4, (byte) 0x5a,
            (byte) 0xaf, (byte) 0xb2, (byte) 0x72, (byte) 0xDC};

    /*披克BLE蓝牙 16个字节的长度的握手秘钥  */
    byte[] PEAKE_BLE_SHAKE_KEY = {
            (byte) 0x2E, (byte) 0xCD, (byte) 0xB4, (byte) 0xAC,
            (byte) 0xA6, (byte) 0x32, (byte) 0xEE, (byte) 0x9F,
            (byte) 0x86, (byte) 0x2E, (byte) 0xEE, (byte) 0x9F,
            (byte) 0xB6, (byte) 0xF2, (byte) 0x68, (byte) 0xDC};
    ...
    int CrcTable[] = {0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0,
    ...
    byte[] crc16_tab_h = {(byte) 0x00, (byte) 0xC1, (byte) 0x81,
     ...
     byte[] crc16_tab_l = {(byte) 0x00, (byte) 0xC0, (byte) 0xC1,
