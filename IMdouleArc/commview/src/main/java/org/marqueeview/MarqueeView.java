...
public class MarqueeView extends ViewFlipper implements LifecycleObserver {
    ...
    private int gravity = Gravity.LEFT | Gravity.FILL_VERTICAL;
    ...
    private List<MarQueeData> messages = new ArrayList<>();
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CommViewMarqueeViewStyle, defStyleAttr, 0);

        interval = typedArray.getInteger(R.styleable.CommViewMarqueeViewStyle_mvInterval, interval);
        hasSetAnimDuration = typedArray.hasValue(R.styleable.CommViewMarqueeViewStyle_mvAnimDuration);
        animDuration = typedArray.getInteger(R.styleable.CommViewMarqueeViewStyle_mvAnimDuration, animDuration);
        singleLine = typedArray.getBoolean(R.styleable.CommViewMarqueeViewStyle_mvSingleLine, false);
        if (typedArray.hasValue(R.styleable.CommViewMarqueeViewStyle_mvTextSize)) {
            textSize = (int) typedArray.getDimension(R.styleable.CommViewMarqueeViewStyle_mvTextSize, textSize);
            textSize = Utils.px2sp(context, textSize);
        }
        textColor = typedArray.getColor(R.styleable.CommViewMarqueeViewStyle_mvTextColor, textColor);
        @FontRes int fontRes = typedArray.getResourceId(R.styleable.CommViewMarqueeViewStyle_mvFont, 0);
        if (fontRes != 0) {
            typeface = ResourcesCompat.getFont(context, fontRes);
        }
        int gravityType = typedArray.getInt(R.styleable.CommViewMarqueeViewStyle_mvGravity, GRAVITY_LEFT);
        switch (gravityType) {
            case GRAVITY_LEFT:
                gravity = Gravity.LEFT | Gravity.FILL_VERTICAL;
                break;
            case GRAVITY_CENTER:
                gravity = Gravity.CENTER;
                break;
            case GRAVITY_RIGHT:
                gravity = Gravity.RIGHT | Gravity.FILL_VERTICAL;
                break;
        }

        if (typedArray.hasValue(R.styleable.CommViewMarqueeViewStyle_mvDirection)) {
            direction = typedArray.getInt(R.styleable.CommViewMarqueeViewStyle_mvDirection, direction);
            switch (direction) {
     ...
     */
    public void startWithList(List<MarQueeData> messages) {
     ...
     * @param outAnimResID 离开动画的resID
     */
    public void startWithList(List<MarQueeData> messages, @AnimRes int inAnimResId, @AnimRes int outAnimResID) {
    ...
    private void start(final @AnimRes int inAnimResId, final @AnimRes int outAnimResID) {
        ...
        position = 0;
        addView(createTextView(messages.get(position).getTitle()));
        ...
        if (getInAnimation() != null) {
            getInAnimation().setAnimationListener(new Animation.AnimationListener() {
                ...
                public void onAnimationEnd(Animation animation) {
                    ...
                    View view = createTextView(messages.get(position).getTitle());
    ...
    private TextView createTextView(String marqueeItem) {
        //LayoutInflater.from(mContext).inflate(R.layout.comm_view_marquee_layout)
    ...
    public List<MarQueeData> getMessages() {
        return messages;
    }

    public void setMessages(List<MarQueeData> messages) {
